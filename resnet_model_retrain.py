...
def _variable_on_cpu(data, addr, scope_name, name, shape, initializer):
...
def conv2d_fixed_padding(inputs, filters, kernel_size, strides, data_format, name, scope_name, addr, data):
  ...
  if data_format == 'channels_last':
	...
	pass_string = 'NHWC'
  kernel = _variable_on_cpu(data, addr, scope_name, 'weights_'+name, [kernel_size, kernel_size, inp_channels, filters] ,tf.variance_scaling_initializer()) 
...
def building_block(inputs, filters, is_training, projection_shortcut, strides,
  ...
  inputs = conv2d_fixed_padding(
      data=data, addr=addr, scope_name=scope_name, inputs=inputs, filters=filters, kernel_size=3, strides=strides,
...
def bottleneck_block(inputs, filters, is_training, projection_shortcut,
  ...
  inputs = conv2d_fixed_padding(
      		data=data, addr=addr, scope_name=scope_name,inputs=inputs, filters=filters, kernel_size=1, strides=1,
      		data_format=data_format, name= name+'_conv1')
...
def block_layer(inputs, filters, block_fn, blocks, strides, is_training, name,
  ...
  def projection_shortcut(inputs):
    	return conv2d_fixed_padding(
        	data=data, addr=addr, scope_name=scope_name,inputs=inputs, filters=filters_out, kernel_size=1, strides=strides,
...
def cifar10_resnet_v2_generator(resnet_size, num_classes, data_format=None):
  ...
  def model(inputs, is_training):
    ...
    with tf.variable_scope('conv1' , reuse=None) as scope:
      inputs = conv2d_fixed_padding(
        data=data, addr=addr, scope_name='conv1', inputs=inputs, filters=16, kernel_size=3, strides=1,
