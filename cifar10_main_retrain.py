  ...
  if((load_done == 0) and RETRAIN):
	...
	saver = tf.train.Saver()
  	with tf.Session() as sess:
		...
		saver.restore(sess, model_dir_saved +'/'+FLAGS.ckpt_file)
		...
		#	var = tf.get_variable(i.split(":")[0])
		#	sess.run(var.assign(data[i][addr[i]]))

		#sess.run(tf.global_variables_initializer())
		saver.save(sess, FLAGS.model_dir + '/model.ckpt')
		#saver = tf.train.Saver(var_list=tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES), write_version=tf.train.SaverDef.V2)
		#save(saver, sess, FLAGS.model_dir)
